  ...
  <div class="mod-config">
      ...
      <div>
        <el-form-item label="下发指令">
          <el-button
            ...
            type="primary"
            @click="releaseCommand(command)"
            :disabled="
              (command.commandType !== 3 && dataListSelections.length <= 0) ||
              (command.commandType === 3 && dataListSelections.length !== 1)
            "
          >
            {{ command.name }}
          </el-button>
    ...
    >
      ...
      >
        <template slot-scope="scope">
          ...
          <el-tag v-if="scope.row.status === 2" size="small" type="warning"
            >待下发</el-tag
          >
          <el-tag v-if="scope.row.status === 3" size="small" type="waning"
            >待安装</el-tag
          >
          <el-tag v-if="scope.row.status === 4" size="small">工作中</el-tag>
          <el-tag v-if="scope.row.status === 5" size="small" type="warning"
            >已回收</el-tag
          >
        </template>
      </el-table-column>
      ...
      >
        <template slot-scope="scope">
          <el-tag
            v-if="scope.row.status === 0 || !scope.row.ip"
            size="small"
            type="danger"
            >离线</el-tag
          >
          <el-tag v-if="scope.row.status === 1 && scope.row.ip" size="small"
            >在线</el-tag
          >
        </template>
      </el-table-column>
      ...
      >
        <template slot-scope="scope">
          <el-button
            v-if="isAuth('equipment:equip:update')"
            type="text"
            size="small"
          ...
          <el-button
            ...
            type="text"
            size="small"
            @click="old(scope.row.id, scope.row.name)"
            >　老化
          </el-button>
          <el-button
            v-if="
              isAuth('equipment:equip:release') &&
              scope.row.status === 2 &&
              checkIfRelease(scope.row)
            "
            type="text"
            size="small"
            @click="releaseEquip(scope.row.id, scope.row.name)"
            >下发
          </el-button>
          <el-button
            v-if="isAuth('equipment:equip:delete')"
          ...
          <el-button
            v-if="isAuth('equipment:equip:recycle') && scope.row.status !== 3"
            type="text"
            size="small"
            @click="recycleHandle(scope.row.id, scope.row.name)"
            ><span style="color: lightpink">回收</span>
          </el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    getDataList () {
      ...
      this.$http({
        ...
        params: this.$http.adornParams({
          ...
          expiredTimeFrom: this.dataForm.expiredTimeRange
            ? this.dataForm.expiredTimeRange[0].getTime()
            : null,
